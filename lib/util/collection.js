...
// returns new object with certain keys dropped. Great for redaction and filtering.
const objectLessKeys = (fst = {}, keys = []) => {
  const map = lookupMap(keys);
  return Object.keys(fst).reduce((obj, key) => {
    if (!map[key]) {
      obj[key] = fst[key]
    }
    return obj;
...
const arrayUnion = (...arrs) => {
  return uniqueArray(arrs.flat());
};
...
const arrayMutualDiff = (a, b) => {
  return uniqueArray([...arrayDiff(a, b), ...arrayDiff(b, a)])
};
