...
const backfillEntityInfoDefaults = (obj, ctx, entityInfo) => {
  ...
  obj.entityName = entityInfo.entityName;
  obj.entityNamePlural = entityInfo.entityNamePlural;
  obj.entityLabel = entityInfo.entityLabel;
  obj.entityLabelPlural = entityInfo.entityLabelPlural;
  obj.pascalName = pascalEntity;
  obj.createColumns = arrayIntersect(ctx.createColumns || [], entityInfo.columns);
  obj.updateColumns = arrayIntersect(ctx.updateColumns || [], entityInfo.columns);
  obj.selectColumns = arrayIntersect(ctx.selectColumns || [], entityInfo.columns);
  obj.keys = _makeKeys(arrayIntersect(ctx.key || ['id'], entityInfo.key));
  obj.selectArgs = _makeSelectArgs(ctx.selectColumns, entityName);
  obj.aliasLookup = _makeAliasLookup(obj.selectArgs);
  obj.joins = ctx.joins || [];
  obj.joinTypesOrdered = _getJoinCallsInOrder(ctx.joins || []);
  obj.validators = ctx.validators || {};
  obj.listArgs = _getListArgs(ctx);
 ...
 *   overrides the papa bear ctx object. A string can be used that
 *   identifies an action as defined by onRedux('entityAction')
 */
const modelFromContext = (ctx, fns) => {
  ...
  backfillEntityInfoDefaults(entityInfo, ctx, entityInfo);
  ...
  backfillEntityInfoDefaults(iface, ctx, entityInfo);
